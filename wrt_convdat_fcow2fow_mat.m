function [iok]=wrt_convdat_fcow2fow_mat(outname, nang, nlay, ngas, nchan, gasid,...
   secang, comment, temp, amount, freq, idchan, res, rnfwhm, ctrans, mfiles);

% function [iok]=wrt_convdat_fcow2fow(outname,nang,nlay,ngas, nchan, gasid, ...
...
% binary data file for some profile. Converts fcow to fow by treating "fc"
% as "f".
%
% Input:
...
% Update: 31 Jul 2000, S.Hannon
% Update: 14 Apr 2009, S.Hannon - fcow2fow variant created
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% fcow to fow conversion variables
ngasm1= ngas - 1; % skip one gas/set
gasindkeep=[1, 3:ngas]; % skip 2nd gas amount
irowm1 = nang*nlay*ngasm1; % number of rows for output
irowkeep =(1:irowm1) + (nang*nlay); % skip 1st trans set
...
gasID = gasid(gasindkeep);
secangs = secang(1:nang);
T = temp(1:nlay);
Q = amount(1:nlay,gasindkeep);

L2Sdata.f  = freq(1:nchan);
L2Sdata.idchan = idchan(1:nchan);
L2Sdata.res    = res(1:nchan);
L2Sdata.rnfwhm = rnfwhm(1:nchan);
L2Sdata.ctrans  = ctrans(irowkeep,1:nchan);

strall = [' nang nlay ngasm1 nchan gasID secangs comment T Q L2Sdata gasindkeep irowkeep mfiles'];
